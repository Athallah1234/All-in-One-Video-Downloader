...
class PlaylistDialog(QDialog):
    ...
    def __init__(self, playlist: dict, selected: list[int] | None = None, parent=None):
        ...
        self.setWindowTitle("Choose Playlist Items")
        self.resize(940, 650)
        ...
        root.addWidget(QLabel(f"<h2>{playlist.get('title') or 'Playlist'}</h2>"))
        root.addWidget(QLabel(f"{playlist.get('uploader') or playlist.get('channel') or 'Unknown uploader'} · {len(self.entries)} item(s)"))
        tools = QHBoxLayout()
        ...
        tools.addWidget(self.search, 1); tools.addWidget(self.availability); tools.addWidget(select); tools.addWidget(none); tools.addWidget(invert); root.addLayout(tools)
        self.table = QTableWidget(0, 6)
        self.table.setHorizontalHeaderLabels(["Download", "#", "Title", "Duration", "Uploader", "ID"])
        ...
        self.search.textChanged.connect(self.apply_filter); self.availability.currentIndexChanged.connect(self.apply_filter)
    ...
    def populate_batch(self):
        ...
        for offset in range(self._next_entry, end):
            ...
            checked = index in self.initial_selected if self.initial_selected else self.is_available(entry); check.setCheckState(Qt.Checked if checked else Qt.Unchecked)
            if not self.is_available(entry): check.setToolTip("Item appears unavailable and is not selected by default.")
            self.table.setItem(row, 0, check)
            values = (index, entry.get("title") or "Unavailable / untitled item", format_duration(entry.get("duration")), entry.get("uploader") or entry.get("channel") or "—", entry.get("id") or "—")
    ...
    def apply_filter(self):
        term = self.search.text().strip().casefold(); mode = self.availability.currentIndex()
        for row in range(self.table.rowCount()):
            ...
            haystack = " ".join(self.table.item(row, col).text() for col in (1, 2, 4, 5)).casefold()
            visible = term in haystack and (mode == 0 or (mode == 1 and available) or (mode == 2 and not available))
...
class BatchUrlDialog(QDialog):
    ...
    def __init__(self, service, parent=None):
        ...
        hint = QLabel("Paste one HTTP/HTTPS URL per line, or load a UTF-8/UTF-16 .txt file. Blank lines and lines beginning with # are ignored; duplicate URLs are analyzed once.")
    ...
    def start_analysis(self):
        ...
        urls, invalid, duplicates = parse_batch_urls(self.input.toPlainText())
        if not urls:
            QMessageBox.warning(self, "No valid URLs", "Enter at least one valid HTTP or HTTPS URL."); return
        self.records = [{"url": url, "status": "Pending", "info": None, "error": "", "playlist_items": []} for url in urls]
        ...
        invalid_preview = "; ".join(invalid[:3]); details = f"{len(urls)} unique valid URL(s) · {duplicates} duplicate(s) skipped · {len(invalid)} invalid line(s)"
    ...
    def schedule_more(self):
        ...
        while not self.stopped and self.pending and self.active < limit:
            ...
            worker = MetadataWorker(self.service, record["url"]); self.workers[row] = worker; self.active += 1
    ...
    def analysis_succeeded(self, row, info):
        ...
        record = self.records[row]; record["info"] = info; record["status"] = "Ready"; entries = info.get("entries"); count = len(entries) if isinstance(entries, list) else 0; media_type = "Playlist" if count else (info.get("media_type") or "Media")
    ...
    def accept_results(self):
        ...
        self._results = [{"url": self.records[row]["url"], "info": self.records[row]["info"], "playlist_items": list(self.records[row]["playlist_items"])} for row in chosen]
        self.accept()
