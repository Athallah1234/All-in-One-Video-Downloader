...
from services.system_state_service import SystemStateService,automatic_pause_reasons
...
from utils.extractor_args import parse_extractor_args
from utils.ffmpeg_filters import AUDIO_ENCODERS,AUDIO_FILTER_PRESETS,VIDEO_ENCODERS,VIDEO_FILTER_PRESETS,validate_encoder_container,validate_filtergraph
from utils.sponsorblock import SPONSORBLOCK_CATEGORIES,validate_sponsorblock
...
class DownloaderTab(QWidget):
    ...
    def __init__(self,service,repo,settings,logger,parent=None):
        super().__init__(parent); self.service=service; self.repo=repo; self.settings=settings; self.logger=logger; self.pool=QThreadPool.globalInstance(); self.estimate_pool=QThreadPool(self); self.estimate_pool.setMaxThreadCount(3); self.info=None; self.analyzed_url="";self.analyzed_extractor_args={}; self.playlist_selection=[]; self.workers={}; self.estimators={}; self.queue=[]; self.queue_run_enabled=str(settings.value("downloads/auto_start",True)).lower() in {"true","1","yes"}; self.speed_ema=None; self.postprocess_ema=10.0; self.net=QNetworkAccessManager(self); self.eta_timer=QTimer(self); self.eta_timer.setInterval(1000); self.eta_timer.timeout.connect(self.update_queue_eta); self.eta_timer.start()
        self.global_manual_pause=False;self.global_policy_reasons=set();self.global_policy_override=False;self.globally_paused_ids=set();self.system_state_service=SystemStateService();self.system_monitor=QTimer(self);self.system_monitor.timeout.connect(self.check_global_pause_policy);self.system_monitor.setInterval(max(2,int(settings.value("downloads/system_monitor_seconds",10)))*1000);self.system_monitor.start()
        ...
        self.drm_status=QLabel("Analyze media to inspect format-level DRM");self.drm_status.setWordWrap(True);form.addRow("Format protection",self.drm_status)
        ...
        self.video_filter=QLineEdit();self.video_filter.setMaxLength(4096);self.video_filter.setPlaceholderText("Example: scale=-2:1080,unsharp=5:5:0.7");self.video_filter.setToolTip("FFmpeg -vf filtergraph. Passed as one argument directly to FFmpeg; no command shell is used.");self.video_filter_preset=QComboBox();[self.video_filter_preset.addItem(label,value) for label,value in VIDEO_FILTER_PRESETS];self.video_filter_preset.currentIndexChanged.connect(lambda:self.video_filter.setText(self.video_filter_preset.currentData() or ""));video_filter_row=QHBoxLayout();video_filter_row.addWidget(self.video_filter,1);video_filter_row.addWidget(self.video_filter_preset);form.addRow("Custom video filter",video_filter_row)
        self.audio_filter=QLineEdit();self.audio_filter.setMaxLength(4096);self.audio_filter.setPlaceholderText("Example: loudnorm=I=-16:LRA=11:TP=-1.5");self.audio_filter.setToolTip("FFmpeg -af filtergraph. Applying it re-encodes only the first audio stream.");self.audio_filter_preset=QComboBox();[self.audio_filter_preset.addItem(label,value) for label,value in AUDIO_FILTER_PRESETS];self.audio_filter_preset.currentIndexChanged.connect(lambda:self.audio_filter.setText(self.audio_filter_preset.currentData() or ""));audio_filter_row=QHBoxLayout();audio_filter_row.addWidget(self.audio_filter,1);audio_filter_row.addWidget(self.audio_filter_preset);form.addRow("Custom audio filter",audio_filter_row)
        self.video_filter_encoder=QComboBox();[self.video_filter_encoder.addItem(label,value) for label,value in VIDEO_ENCODERS];form.addRow("Filter video encoder",self.video_filter_encoder);self.audio_filter_encoder=QComboBox();[self.audio_filter_encoder.addItem(label,value) for label,value in AUDIO_ENCODERS];form.addRow("Filter audio encoder",self.audio_filter_encoder)
        validate_filters=QPushButton("Validate FFmpeg filters");self.filter_status=QLabel("No custom filters");self.filter_status.setWordWrap(True);filter_footer=QHBoxLayout();filter_footer.addWidget(self.filter_status,1);filter_footer.addWidget(validate_filters);form.addRow(filter_footer)
        self.sponsorblock_enabled=QCheckBox("Use SponsorBlock community segments");form.addRow(self.sponsorblock_enabled)
        self.sponsorblock_categories=QTableWidget(len(SPONSORBLOCK_CATEGORIES),3);self.sponsorblock_categories.setHorizontalHeaderLabels(["Category","Mark","Remove"]);self.sponsorblock_categories.horizontalHeader().setSectionResizeMode(0,QHeaderView.Stretch);self.sponsorblock_categories.horizontalHeader().setSectionResizeMode(1,QHeaderView.ResizeToContents);self.sponsorblock_categories.horizontalHeader().setSectionResizeMode(2,QHeaderView.ResizeToContents);self.sponsorblock_categories.verticalHeader().hide();self.sponsorblock_categories.setMaximumHeight(265);self.sponsorblock_categories.setSelectionMode(QAbstractItemView.NoSelection)
        self.sponsorblock_checks={}
        for row,(key,label,removable) in enumerate(SPONSORBLOCK_CATEGORIES):
            item=QTableWidgetItem(label);item.setData(Qt.UserRole,key);item.setFlags(item.flags()&~Qt.ItemIsEditable);self.sponsorblock_categories.setItem(row,0,item);mark=QCheckBox();remove=QCheckBox();remove.setEnabled(removable);remove.setToolTip("This SponsorBlock category is mark-only" if not removable else "Physically remove this category from the downloaded media");mark.toggled.connect(lambda checked,r=remove: r.setChecked(False) if checked else None);remove.toggled.connect(lambda checked,m=mark: m.setChecked(False) if checked else None);self.sponsorblock_categories.setCellWidget(row,1,mark);self.sponsorblock_categories.setCellWidget(row,2,remove);self.sponsorblock_checks[key]=(mark,remove)
        self.sponsorblock_checks["sponsor"][1].setChecked(True);form.addRow("SponsorBlock categories",self.sponsorblock_categories)
        self.sponsorblock_api=QLineEdit("https://sponsor.ajay.app");self.sponsorblock_api.setMaxLength(2048);self.sponsorblock_api.setToolTip("SponsorBlock-compatible API base URL. yt-dlp sends the supported video identifier and requested categories; browser cookies and account credentials are not sent to this API.");form.addRow("SponsorBlock API",self.sponsorblock_api)
        self.sponsorblock_chapter_title=QLineEdit("[SponsorBlock]: %(category_names)l");self.sponsorblock_chapter_title.setMaxLength(512);self.sponsorblock_chapter_title.setToolTip("Allowed fields: start_time, end_time, category, categories, name, category_names");form.addRow("Marked chapter title",self.sponsorblock_chapter_title)
        self.sponsorblock_force_keyframes=QCheckBox("Force keyframes at cuts (more accurate, slower)");self.sponsorblock_force_keyframes.setToolTip("Improves cut accuracy but can substantially increase FFmpeg processing time.");form.addRow(self.sponsorblock_force_keyframes)
        validate_sponsor=QPushButton("Validate SponsorBlock");self.sponsorblock_status=QLabel("Disabled");self.sponsorblock_status.setWordWrap(True);sponsor_footer=QHBoxLayout();sponsor_footer.addWidget(self.sponsorblock_status,1);sponsor_footer.addWidget(validate_sponsor);form.addRow(sponsor_footer)
        self.extractor_args_input=QPlainTextEdit();self.extractor_args_input.setPlaceholderText("Optional, one per line: youtube:player_client=web,android;player_skip=webpage");self.extractor_args_input.setMaximumHeight(72);self.extractor_args_input.setMaximumBlockCount(64);validate_args=QPushButton("Validate");self.extractor_args_status=QLabel("No custom extractor arguments");args_row=QVBoxLayout();args_row.addWidget(self.extractor_args_input);args_footer=QHBoxLayout();args_footer.addWidget(self.extractor_args_status,1);args_footer.addWidget(validate_args);args_row.addLayout(args_footer);form.addRow("Per-URL extractor args",args_row)
        ...
        queuebar=QHBoxLayout(); queuebar.addWidget(QLabel("<b>Download Queue</b>")); self.queue_total_label=QLabel("Total estimate: —"); self.queue_eta_label=QLabel("Queue ETA: —");self.global_pause_status=QLabel("Global pause: off");self.global_pause_button=QPushButton("Pause All");self.global_pause_button.setToolTip("Pause every active download and prevent queued downloads from starting (Ctrl+Shift+P)"); startall=QPushButton("Start All"); cancelall=QPushButton("Cancel All"); clearcompleted=QPushButton("Clear Completed"); queuebar.addWidget(self.queue_total_label); queuebar.addWidget(self.queue_eta_label);queuebar.addWidget(self.global_pause_status); queuebar.addStretch(); [queuebar.addWidget(x) for x in (self.global_pause_button,startall,cancelall,clearcompleted)]; root.addLayout(queuebar)
    ...
    def apply_saved_defaults(self):
        if hasattr(self,"system_monitor"):self.system_monitor.setInterval(max(2,int(self.settings.value("downloads/system_monitor_seconds",10)))*1000)
    ...
    def open_batch(self):
        ...
        for result in results:added+=bool(self.enqueue_request(result["url"],result["info"],result["playlist_items"],start=False,extractor_args=result.get("extractor_args") or {}))
    ...
    def analyze(self):
        ...
        parsed=self.validate_extractor_args_ui(False)
        if parsed is None:return
        self.info=None; self.analyzed_url=url;self.analyzed_extractor_args=parsed; self.playlist_selection=[]; self.playlist_button.setEnabled(False); self.playlist_label.setText("Analyzing…")
        self.status.emit("Analyzing…"); self.title.setText("Analyzing…"); worker=MetadataWorker(self.service,url,parsed); worker.signals.metadata.connect(self.show_info); worker.signals.failed.connect(self.fail_analysis); self.pool.start(worker)
    def validate_extractor_args_ui(self,show_success=False):
        try:result=parse_extractor_args(self.extractor_args_input.toPlainText())
        except ValueError as exc:self.extractor_args_status.setText(str(exc));QMessageBox.warning(self,"Invalid extractor arguments",str(exc));return None
        self.extractor_args_status.setText("No custom extractor arguments" if not result.specifications else f"Valid · {result.specifications} extractor(s), {result.arguments} argument(s)")
        if show_success and result.specifications:QMessageBox.information(self,"Extractor arguments valid","The arguments are structurally valid. Extractor-specific support is checked by yt-dlp during analysis.")
        return result.value
    def refresh_filter_controls(self):
        video=self.kind.currentText() in {"Video","Video Only"};audio=self.kind.currentText() in {"Video","Audio Only"}
        for widget in (self.video_filter,self.video_filter_preset,self.video_filter_encoder):widget.setEnabled(video)
        for widget in (self.audio_filter,self.audio_filter_preset,self.audio_filter_encoder):widget.setEnabled(audio)
    def validate_filters_ui(self,show_success=False):
        video=self.video_filter.text().strip() if self.kind.currentText() in {"Video","Video Only"} else "";audio=self.audio_filter.text().strip() if self.kind.currentText() in {"Video","Audio Only"} else ""
        for label,value in (("Video",video),("Audio",audio)):
            report=validate_filtergraph(value)
            if not report.valid:self.filter_status.setText(f"{label}: {report.error}");QMessageBox.warning(self,"Invalid FFmpeg filter",f"{label} filter: {report.error}");return None
        extension=self.audio.currentText() if self.kind.currentText()=="Audio Only" else (self.container.currentData() or "")
        if extension:
            report=validate_encoder_container(self.video_filter_encoder.currentData(),self.audio_filter_encoder.currentData(),extension,bool(video),bool(audio))
            if not report.valid:self.filter_status.setText(report.error);QMessageBox.warning(self,"Incompatible filter encoder",report.error);return None
        self.filter_status.setText("Valid · selected streams will be re-encoded" if video or audio else "No custom filters")
        if show_success and (video or audio):QMessageBox.information(self,"FFmpeg filters valid","The filtergraph is structurally valid. FFmpeg will verify filter names and options when processing starts. Only filtered streams are re-encoded; other streams are copied.")
        return video,audio
    def refresh_sponsorblock_controls(self):
        media=self.kind.currentText() in {"Video","Video Only","Audio Only"};self.sponsorblock_enabled.setEnabled(media);enabled=media and self.sponsorblock_enabled.isChecked()
        for widget in (self.sponsorblock_categories,self.sponsorblock_api,self.sponsorblock_chapter_title,self.sponsorblock_force_keyframes):widget.setEnabled(enabled)
        if not media:self.sponsorblock_status.setText("Unavailable for auxiliary-only downloads")
        elif not enabled:self.sponsorblock_status.setText("Disabled")
    def validate_sponsorblock_ui(self,show_success=False):
        enabled=self.sponsorblock_enabled.isChecked()
        if enabled and self.kind.currentText() not in {"Video","Video Only","Audio Only"}:QMessageBox.warning(self,"SponsorBlock unavailable","SponsorBlock marking and removal require a downloaded media file.");return None
        mark={key for key,(mark,_) in self.sponsorblock_checks.items() if mark.isChecked()};remove={key for key,(_,remove) in self.sponsorblock_checks.items() if remove.isChecked()}
        if enabled and not (mark or remove):QMessageBox.warning(self,"SponsorBlock categories required","Select at least one category to mark or remove.");return None
        report=validate_sponsorblock(mark,remove,self.sponsorblock_api.text(),self.sponsorblock_chapter_title.text())
        if not report.valid:self.sponsorblock_status.setText(report.error);QMessageBox.warning(self,"Invalid SponsorBlock configuration",report.error);return None
        self.sponsorblock_status.setText("Disabled" if not enabled else f"Valid · mark {len(mark)} · remove {len(remove)}")
        if show_success and enabled:QMessageBox.information(self,"SponsorBlock configuration valid","Segments will be requested by yt-dlp for every supported video. Remove takes precedence over marking. Missing community segments do not fail the download.")
        return enabled,mark,remove
    ...
    def show_info(self,info):
        if not isinstance(info,dict):self.fail_analysis("No metadata was returned for this URL");return
        self.info=info; entries=info.get("entries"); count=len(entries) if isinstance(entries,list) else info.get("playlist_count")
        is_playlist=isinstance(entries,list) and bool(entries);is_channel=bool(info.get("_app_is_channel"))
        self.playlist_button.setEnabled(is_playlist)
        self.playlist_button.setText("Choose Channel Videos…" if is_channel else "Choose Playlist Items…")
        limit_note=" · limit reached" if info.get("_app_channel_truncated") else "";self.playlist_label.setText((f"Channel · {count} loaded{limit_note} · selection required" if is_playlist else "Channel · no selectable videos returned") if is_channel else (f"{count} items · selection required" if is_playlist else "Not a playlist"))
        self.title.setText(info.get("channel") or info.get("title") or "Untitled media"); fields=[("Type","Channel" if is_channel else ("Playlist" if is_playlist else "Media")),("Channel",info.get("channel") or info.get("uploader")),("Channel ID",info.get("channel_id") or info.get("uploader_id")),("Followers",info.get("channel_follower_count")),("Extractor",info.get("extractor_key") or info.get("extractor")),("Duration",format_duration(info.get("duration"))),("Views",info.get("view_count")),("Upload date",info.get("upload_date")),("Media ID",info.get("id")),("Loaded items",count),("Analysis limit",info.get("_app_channel_limit") if is_channel else None),("Availability",info.get("availability"))]; self.meta.setPlainText("\n".join(f"{k}: {v if v is not None else '—'}" for k,v in fields)+"\n\n"+(info.get("description") or ""))
        self.current_formats=info.get("formats") or [];report=info.get("_app_drm_report") or drm_report(self.current_formats);self.drm_report=report
        if not self.current_formats:self.drm_status.setText("DRM: format details unavailable during flat collection analysis")
        elif report["all_media_blocked"]:self.drm_status.setText(f"⛔ All {report['total']} media formats are DRM-protected or suspected DRM");self.drm_status.setStyleSheet("color:#ef4444;font-weight:600")
        elif report["confirmed"] or report["suspected"]:self.drm_status.setText(f"⚠ DRM detected: {report['confirmed']} confirmed, {report['suspected']} possible; {report['clear']} clear, {report['unknown']} unknown");self.drm_status.setStyleSheet("color:#eab308;font-weight:600")
        else:self.drm_status.setText(f"DRM: none reported · {report['clear']} clear, {report['unknown']} unknown");self.drm_status.setStyleSheet("color:#22c55e")
        self.current_is_playlist=is_playlist; self.populate_formats(self.current_formats); self.populate_resolutions(self.current_formats,is_playlist); self.populate_codecs(self.current_formats,is_playlist); self.populate_fps(self.current_formats,is_playlist); self.populate_bit_depths(self.current_formats,is_playlist); self.populate_dynamic_ranges(self.current_formats,is_playlist); self.populate_audio_codecs(self.current_formats,is_playlist);self.apply_saved_defaults(); thumb=info.get("thumbnail");
        if thumb:
    ...
    def choose_playlist_items(self):
        ...
        self.playlist_label.setText(f"{'Channel · ' if (self.info or {}).get('_app_is_channel') else ''}{len(self.playlist_selection)} of {total} selected")
    ...
    def populate_formats(self,formats):
        ...
        for f in formats:
            fid=str(f.get("format_id") or "");ext=f.get("ext") or "?";res=f.get("resolution") or (f"{f.get('height')}p" if f.get("height") else "audio");note=f.get("format_note") or "";size=format_size(f.get("filesize") or f.get("filesize_approx"));drm=detect_format_drm(f)
            suffix=f" · ⚠ {drm.label}" if drm.blocked else (" · DRM unknown" if drm.level=="unknown" else " · Clear")
            self.format.addItem(f"{fid} · {ext} · {res} · {note} · {size}{suffix}",fid);item=self.format.model().item(self.format.count()-1);item.setEnabled(not drm.blocked)
            item.setToolTip(("Blocked before download. Evidence: "+", ".join(drm.evidence)) if drm.blocked else ("Extractor explicitly reports this format as DRM-free." if drm.level=="clear" else "The extractor did not provide a DRM signal for this format."))
            color={"confirmed":"#ef4444","suspected":"#eab308","clear":"#22c55e","unknown":"#9ca3af"}[drm.level];item.setForeground(QBrush(QColor(color)))
    ...
    def add_queue(self):
        ...
        if self.info.get("_app_is_channel") and (not isinstance(entries,list) or not entries):QMessageBox.warning(self,"No channel videos","The channel analysis returned no selectable videos. Check the channel URL, authentication, selected tab, or analysis log.");return
    ...
    def enqueue_request(self,url,info,playlist_items,start=True,extractor_args=None):
        playlist_items=list(playlist_items);extractor_args=self.analyzed_extractor_args if extractor_args is None else extractor_args
        if info.get("_app_is_channel") and not playlist_items:QMessageBox.warning(self,"Channel selection required","Select at least one analyzed channel video before adding it to the queue. Whole-channel download is never implicit.");return False
        if url==self.analyzed_url:
            current=self.validate_extractor_args_ui(False)
            if current is None:return False
            if current!=self.analyzed_extractor_args:QMessageBox.warning(self,"Analyze again","Extractor arguments changed after analysis. Analyze the URL again so format and playlist metadata match the queued request.");return False
        custom_filters=self.validate_filters_ui(False)
        if custom_filters is None:return False
        sponsorblock=self.validate_sponsorblock_ui(False)
        if sponsorblock is None:return False
        ...
        formats=info.get("formats") or [];protection=info.get("_app_drm_report") or drm_report(formats)
        required=("audio",) if self.kind.currentText()=="Audio Only" else (("video",) if self.kind.currentText()=="Video Only" else ("video","audio"))
        blocked_streams=[stream for stream in required if all_stream_formats_blocked(formats,stream)]
        if formats and blocked_streams:QMessageBox.critical(self,"DRM-protected media",f"All analyzed {' and '.join(blocked_streams)} formats are marked as DRM or possible DRM. This application will not queue or attempt to bypass protected streams.");return False
        blocked_selection=selector_uses_blocked_format(selector,formats)
        if blocked_selection:QMessageBox.critical(self,"DRM format blocked",f"The selected format is {blocked_selection.label.lower()} and cannot be queued. Evidence: {', '.join(blocked_selection.evidence) or 'extractor DRM flag'}");return False
        ...
        if any(custom_filters) and not FFmpegService.available():QMessageBox.warning(self,"FFmpeg required","Custom video or audio filters require FFmpeg and FFprobe.");return False
        if sponsorblock[0] and not FFmpegService.available():QMessageBox.warning(self,"FFmpeg required","Embedding SponsorBlock chapters or removing segments requires FFmpeg and FFprobe.");return False
    ...
    def render_queue(self):
        ...
        for row,item in enumerate(self.queue):
            ...
            active=req.id in self.workers;globally_paused=req.id in self.globally_paused_ids
            if globally_paused:pause.setText("Global paused");pause.setToolTip(item.get("global_pause_reason","Paused by global policy"))
            pause.setEnabled(active and not globally_paused and (item["status"].startswith("Downloading") or item["status"] in ("Paused","Pausing…"))); cancel.setEnabled(active); pause.clicked.connect(lambda _checked=False,i=item:self.toggle_pause(i)); cancel.clicked.connect(lambda _checked=False,i=item:self.cancel_item(i)); buttons.addWidget(pause); buttons.addWidget(cancel); self.table.setCellWidget(row,9,controls)
    ...
    def start_all(self):
        for item in self.queue:
            if item["status"] in {"Failed","Cancelled"}:item["status"]="Estimating size…" if item["request"].id in self.estimators else "Waiting"
        self.queue_run_enabled=True;self.start_available()
    def global_pause_active(self):return self.global_manual_pause or (bool(self.global_policy_reasons) and not self.global_policy_override)
    def apply_global_pause(self):
        reasons={"Manual pause"} if self.global_manual_pause else set(self.global_policy_reasons)
        for item in self.queue:
            req=item["request"];worker=self.workers.get(req.id)
            if not worker or worker.pause_event.is_set():continue
            if worker.pause():self.globally_paused_ids.add(req.id);item["status"]="Globally pausing…";item["global_pause_reason"]="; ".join(sorted(reasons))
        self.update_global_pause_status();self.render_queue()
    def resume_globally_paused(self):
        for identifier in list(self.globally_paused_ids):
            worker=self.workers.get(identifier)
            item=next((value for value in self.queue if value["request"].id==identifier),None)
            if worker:worker.resume()
            if item:item["status"]=item.get("download_status","Downloading");item.pop("global_pause_reason",None)
        self.globally_paused_ids.clear();self.update_global_pause_status();self.render_queue();self.start_available()
    def manual_global_pause(self):
        self.global_manual_pause=True;self.global_policy_override=False;self.logger.info("Manual global pause requested");self.apply_global_pause()
    def manual_global_resume(self):
        self.global_manual_pause=False;self.global_policy_override=bool(self.global_policy_reasons);self.logger.info("Manual global resume requested%s"," (automatic policy overridden until recovery)" if self.global_policy_override else "");self.resume_globally_paused()
    def toggle_global_pause(self):
        if self.global_pause_active() or self.globally_paused_ids:self.manual_global_resume()
        else:self.manual_global_pause()
    def check_global_pause_policy(self):
        state=self.system_state_service.snapshot();flag=lambda key,default=False:str(self.settings.value(key,default)).lower() in {"true","1","yes"};was_battery=any(reason.startswith("Battery low") for reason in self.global_policy_reasons)
        reasons=automatic_pause_reasons(state,flag("downloads/pause_when_idle",False),int(self.settings.value("downloads/idle_pause_minutes",30)),flag("downloads/pause_low_battery",False),int(self.settings.value("downloads/low_battery_percent",20)),int(self.settings.value("downloads/battery_resume_hysteresis",5)),was_battery)
        previous=set(self.global_policy_reasons);self.global_policy_reasons=reasons
        if not reasons:self.global_policy_override=False
        if reasons and not self.global_policy_override:self.apply_global_pause()
        elif previous and not reasons and flag("downloads/auto_resume_global",True) and not self.global_manual_pause:self.resume_globally_paused()
        self.update_global_pause_status(state)
    def update_global_pause_status(self,state=None):
        active=self.global_pause_active();reasons=({"Manual pause"} if self.global_manual_pause else self.global_policy_reasons) if active else set();text="Global pause: "+("; ".join(sorted(reasons)) if reasons else ("overridden" if self.global_policy_override else "off"))
        if state is not None:
            telemetry=[]
            if state.idle_seconds is not None:telemetry.append(f"idle {format_duration(state.idle_seconds)}")
            if state.battery_percent is not None:telemetry.append(f"battery {state.battery_percent}%{' AC' if state.on_battery is False else ''}")
            self.global_pause_status.setToolTip(" · ".join(telemetry) if telemetry else "Idle/battery information is unavailable on this system")
        self.global_pause_status.setText(text);resume=active or bool(self.globally_paused_ids);self.global_pause_button.setText("Resume All" if resume else "Pause All");self.global_pause_button.setToolTip(("Resume only downloads paused by Global Pause; individually paused items remain paused" if resume else "Pause every active download and prevent queued downloads from starting")+" (Ctrl+Shift+P)")
    def start_available(self):
        if not self.queue_run_enabled or (self.global_pause_active() if hasattr(self,"global_pause_active") else False):return
    ...
    def start_item(self,item):
        ...
        req=item["request"]; item.update(status="Downloading",started=datetime.now().isoformat(timespec="seconds"),started_monotonic=time.monotonic()); verify=str(self.settings.value("downloads/verify_integrity",True)).lower() not in {"false","0","no"}; retries=int(self.settings.value("downloads/integrity_retries",2));retry_enabled=str(self.settings.value("downloads/failure_retry_enabled",True)).lower() in {"true","1","yes"};failure_retries=int(self.settings.value("downloads/failure_retries",3)) if retry_enabled else 0;worker=DownloadWorker(self.service,req,verify,retries,failure_retries,int(self.settings.value("downloads/retry_backoff_base",2)),int(self.settings.value("downloads/retry_backoff_max",60)),int(self.settings.value("downloads/retry_jitter_percent",10))); self.workers[req.id]=worker
        worker.signals.progress.connect(lambda d,i=item:self.on_progress(i,d)); worker.signals.phase.connect(lambda p,i=item:self.set_status(i,p)); worker.signals.pause_state.connect(lambda paused,i=item:self.on_pause_state(i,paused)); worker.signals.integrity.connect(lambda report,i=item:self.on_integrity(i,report)); worker.signals.retry.connect(lambda attempt,reason,i=item:self.on_integrity_retry(i,attempt,reason));worker.signals.auto_retry.connect(lambda attempt,maximum,delay,reason,i=item:self.on_failure_retry(i,attempt,maximum,delay,reason)); worker.signals.completed.connect(lambda d,i=item:self.finish(i,d)); worker.signals.failed.connect(lambda e,i=item:self.fail(i,e)); worker.signals.cancelled.connect(lambda i=item:self.cancelled(i)); self.pool.start(worker); self.render_queue(); self.active_changed.emit(len(self.workers));return True
    def on_progress(self,item,data):
        item.pop("retry_ready_monotonic",None)
    ...
    def on_failure_retry(self,item,attempt,maximum,delay,reason):
        item["status"]=f"Retrying failed download ({attempt}/{maximum}) in {delay:.1f}s";item["output"]=f"Last error: {reason[:300]}";item["progress"]=0;item["speed"]="Backoff";item["transfer_components"]={};item["postprocessing"]=False;item.pop("postprocess_started",None);item["failure_retry_attempt"]=attempt;item["retry_ready_monotonic"]=time.monotonic()+delay;self.logger.warning("Automatic failure retry %s/%s for %s in %.1fs: %s",attempt,maximum,item["request"].title,delay,reason);self.render_queue()
    def on_pause_state(self,item,paused):
        identifier=item["request"].id;item["status"]=(f"Globally paused: {item.get('global_pause_reason','policy')}" if identifier in self.globally_paused_ids else "Paused") if paused else item.get("download_status","Downloading"); self.logger.info("Download %s: %s", "paused" if paused else "resumed", item["request"].title); self.render_queue()
    def toggle_pause(self,item):
        worker=self.workers.get(item["request"].id)
        if not worker:return
        if item["request"].id in self.globally_paused_ids:return
    ...
    def finish(self,item,data):
        req=item["request"]; self.workers.pop(req.id,None);self.globally_paused_ids.discard(req.id); item.update(status="Completed",progress=100); downloads=data.get("requested_downloads") or []; first=downloads[0] if downloads and isinstance(downloads[0],dict) else {}; path=first.get("filepath") or data.get("filepath") or data.get("_filename"); item["output"]=path or item["output"]
    ...
    def fail(self,item,error):
        req=item["request"]; self.workers.pop(req.id,None);self.globally_paused_ids.discard(req.id); item["status"]="Failed"; self.repo.add(task_id=req.id,title=req.title,original_url=req.url,format=req.format_selector,download_folder=req.folder,started_at=item.get("started"),completed_at=datetime.now().isoformat(timespec="seconds"),status="Failed",error=error); self.logger.error(error);self.notify_user("failed",f"Failed: {req.title}"); self.history_changed.emit(); self.render_queue(); self.active_changed.emit(len(self.workers)); self.start_available()
    ...
    def cancelled(self,item): self.workers.pop(item["request"].id,None);self.globally_paused_ids.discard(item["request"].id); item["status"]="Cancelled";self.notify_user("cancelled",f"Cancelled: {item['request'].title}"); self.render_queue(); self.active_changed.emit(len(self.workers)); self.start_available()
    def cancel_all(self):
